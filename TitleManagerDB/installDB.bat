...
:: SET "DB_ROOT_DIR=C:\Maethor\Projects\TitleManager\TitleManagerDB\"
...
SET "EXECUTION_FILE=execution_log.txt"
SET "DATAINIT_FILE=datainit_log.txt"
...
SET "EXECUTION_ERROR_FILE=error_execution_log.txt"
SET "DATAINIT_ERROR_FILE=error_datainit_log.txt"
...
:: total files to load
SET /a TOTAL_FILES=0
FOR /f "usebackq" %%g IN (`DIR /b "%DATA_DIR%*.csv"`) DO (SET /a TOTAL_FILES=TOTAL_FILES+1)

ECHO Loading data...
FOR /f "usebackq" %%G IN (`DIR /b "%DATA_DIR%*.csv"`) DO ((sqlcmd -b -S H710-MAETHOR\SQLEXPRESS -i %INITIALDATA_SCRIPT% -v DBNAME=%inputDBNAME% SCRIPTNAME=%INITIALDATA_FILE% DIRECTORY="%DATA_DIR%" FILENAME=%%G -o %TMP_FILE%) && (SET /a COUNT=COUNT+1 & CALL :SHOW_PROGRESS !COUNT! %TOTAL_FILES% & CALL :WRITE_FROM_TEMPFILE) || (GOTO :ERROR_DATALOAD))
